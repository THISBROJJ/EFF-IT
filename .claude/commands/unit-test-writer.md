...
Before writing any tests, classify each target in the scope:

- **Haiku** (default): simple functions, CRUD, data transforms, utilities, straightforward conditional logic. Run directly — no sub-agent needed.
- **Sonnet**: async/concurrent code, complex mocking hierarchies, intricate state machines, cryptographic or encoding logic, multi-layered business rules. Spawn a sub-agent with `model: sonnet` scoped to that target only.
- **Opus** (advisor only): test design is architecturally ambiguous — the contract is unclear, the pattern is novel, or the right isolation boundary is non-obvious. Spawn a **read-only** Opus sub-agent to analyze and recommend an approach; then write the actual tests yourself on Haiku. Opus never produces test code.

When a batch mixes simple and complex targets, handle simple targets directly and spawn Sonnet sub-agents for complex ones. Default aggressively to Haiku — escalate only when the logic genuinely warrants it.
...
```bash
SKILL_DIR=".claude/commands/unit-test-writer"
